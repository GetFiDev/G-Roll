...
namespace AppodealAds.Unity.Editor.Utils
{
    ...
    public class iOSPostprocessUtils : MonoBehaviour
    {
        ...
        {
            if (!File.Exists("Assets/Appodeal/Editor/NetworkConfigs/AppodealDependencies.xml"))
            {
                Debug.LogWarning(
                    "Missing Admob config (Assets/Appodeal/Editor/NetworkConfigs/AppodealDependencies.xml).\nAdmob App Id won't be added.");
                return;
            }
        ...
        private static bool CheckiOSAttribute()
        {
            var adMobConfigPath = Path.Combine(Application.dataPath,
                "Appodeal/Editor/NetworkConfigs/AppodealDependencies.xml");
