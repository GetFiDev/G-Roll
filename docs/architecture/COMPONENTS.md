...
|---------|------|----------------|------|
| `UserDatabaseManager.cs` | `/Networks/` | Master user data hub (60KB!) | CRITICAL |
...
Firestore Database
├── /users/{uid}/
│   ├── profile: { displayName, avatar, createdAt }
│   ├── stats: { totalCoins, highScore, gamesPlayed }
│   ├── inventory: { ownedItems[], equippedItems[] }
│   ├── energy: { current, maxEnergy, lastRegenTime }
│   ├── streak: { currentStreak, lastClaimDate }
│   ├── elitePass: { isActive, expiresAt, tier }
│   ├── /sessions/{sessionId}/
│   │   └── { startTime, endTime, score, rewards }
...
├── /appdata/
│   ├── items: { itemCatalog[] }
│   ├── ads: { rewardConfig, dailyLimits }
│   ├── config: { gameSettings, featureFlags }
│   └── version: { minVersion, currentVersion }
│
├── /referralKeys/{code}/
│   └── { ownerUid, usedBy[], usedCount }
│
├── /leaderboards/{period}/
│   └── /entries/{rank}/
...
│
└── /maps/{mapId}/
    └── { name, difficulty, gridData, metadata }
```
...
**Trigger:** Commit message starts with `build:`
...
4. Unity Build (iOS) via game-ci/unity-builder@v4
5. Locate Xcode workspace
6. Unlock keychain (code signing)
7. Fastlane beta (TestFlight upload)
```

### 8.2 Fastlane Configuration
...
### 8.3 Firebase Functions Deployment

**File:** `/functions/deploy.sh`

```bash
# Smart deployment with change detection
# Only deploys modified functions
```

### 8.4 iOS Export Configuration
